...
import {
  runOnboardingSaveRuntimeSmokeTest,
  type OnboardingSmokeReport,
} from "../../tests/user/onboardingSaveRuntime.smoke";
import { Button } from "@/shared/components/Button";
import { Screen } from "@/shared/components/Screen";

export default function OnboardingSaveSmokeTestScreen() {
  const [running, setRunning] = useState(false);
  const [report, setReport] = useState<OnboardingSmokeReport | null>(null);
  ...
  async function handleRun() {
    ...
    try {
      appendLog("Starting onboarding save smoke test");

      const result = await runOnboardingSaveRuntimeSmokeTest({
        onLog: appendLog,
        checkTimeoutMs: 20000,
      });

      setReport(result);
      appendLog(`Completed onboarding save smoke test: ${result.ok ? "PASS" : "FAIL"}`);
    } catch (runError) {
  ...
  return (
    <Screen>
      <ScrollView contentContainerStyle={{ gap: 12, paddingBottom: 24 }}>
        <Text style={{ fontSize: 22, fontWeight: "700" }}>Onboarding Save Smoke Test</Text>
        <Text style={{ color: "#444" }}>
          Validates onboarding save path, UUID generation, offline profile mirror, and sync queue behavior.
        </Text>

        <Button onPress={running ? undefined : handleRun}>
          {running ? "Running..." : "Run Onboarding Save Smoke Test"}
        </Button>
